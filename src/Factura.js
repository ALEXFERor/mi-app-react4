...
function Factura({ venta, total, metodoPago, entregado, vuelto, ruc }) {
  const facturaPDFRef = useRef(null);

  const descargarPDF = () => {
    const input = facturaPDFRef.current;
    ...
    html2canvas(input, { scale: 2, useCORS: true }).then(canvas => {
      ...
      pdf.save(`factura_${venta.num_serie || new Date().toISOString().slice(0, 10)}.pdf`);
  ...
  return (
    <div>
      <button onClick={descargarPDF} className="btn btn-success mt-1 mb-2">Descargar Factura PDF</button>
      <div ref={facturaPDFRef} style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '10px' }}>
          ...
          <p><strong>RUC:</strong> {venta.cliente_dni}</p>
        </div>
